...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    /**
        ...
        @notice sets the inital state of the Bridge to paused.
        @param domainID ID of chain the Bridge contract exists on.
     */
    constructor (uint8 domainID, uint256 expiry) public {
        _domainID = domainID;
    ...
    /**
        ...
        @notice Emits {ProposalEvent} event.
        @notice Emits {FailedExecution} event with the failed reason.
     */
    function executeProposal(uint8 domainID, uint64 depositNonce, bytes calldata data, bytes32 resourceID, bytes calldata signature, bool revertOnFail) public whenNotPaused {
        require(isProposalExecuted(domainID, depositNonce) != true, "Deposit with provided nonce already executed");

        bytes32 message = ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(domainID, depositNonce, data, resourceID)));
        ...
        emit ProposalExecution(domainID, depositNonce, dataHash);
    }
    ...
    function endKeygen(address MPCAddress) external onlyAdmin {
      require(MPCAddress != address(0), "MPC address can't be zero-address");
